#**********************************************************************************************
# Date: 08/21/2020
# Comment(s): R code example for characterizing cycles in ts data.
# Data File(s): returns.txt
#**********************************************************************************************
# Variable Definitions (returns.txt)
# date   = Monthly from January 1926 - December 2008
# ibmrtn = Monthly simple IBM Stock Returns
# vwrtn  = Monthly simple returns for the CRSP equal-weighted index
# ewrtn  = Monthly simple returns of the CRSP equal-weighted index
# sprtn  = Monthly simple returns for the S&P 500
#**********************************************************************************************

# Clear all variables and prior sessions
rm(list=ls(all=TRUE))

# Load Libraries
library(stats)
library(forecast)
library(ggplot2)

# Sample Time-Series Data. 
y1 = ts(rnorm(360,0,1), frequency = 12)
y2 = ts(rnorm(360,0,1) + 20 * sin(2 * pi * (1:360) / 12), frequency = 12)
y3 = ts(rnorm(360,0,1) + 2 * (1:360) / 12, frequency = 12)
y4 = ts(runif(360,0,1), frequency = 12)
tsdisplay(y1) # this does look like a white noise
tsdisplay(y2) # this is not white noise (because of the seasonality)
tsdisplay(y3) # persistent
tsdisplay(y4) # this appears to be white noise
 
quartz()
plot(stl(y2, s.window = "periodic"))

# Autocovariance, ACF, and PACF Functions
quartz()
par(mfrow = c(3,1))
acf(y2, type = "covariance")
acf(y2, type = "correlation", lag.max = 12)
acf(y2, type = "partial")

# Test for white noise using Ljung-Box and Box-Pierce tests
# Small p-value --> Reject H0: yt ~ white noise --> cycles are present
# Ljung-Box Q-Statistic
Box.test(y2, type = "Ljung-Box")
# Box-Pierce Q-Statistic
Box.test(y2, type = "Box-Pierce")


# -------------------------------- [Case Study: IBM vs. SP500] -------------------------------- #
# Series I: Monthly Simple IBM Returns from 1926-2008
z = read.table("returns.txt", header = T)
sibm_ts = ts(z[,2], frequency = 12, start = c(1926,1))
libm_ts = log(1 + sibm_ts) 
quartz()
plot(stl(sibm_ts, s.window = "periodic"))
quartz()
tsdisplay(sibm_ts) # This is pretty much white noice
quartz()
plot(stl(libm_ts, s.window = "periodic"))
quartz()
tsdisplay(libm_ts) # This is pretty much white noice

# Autocovariance, ACF, and PACF Functions
quartz()
par(mfrow = c(3,1))
acf(sibm_ts, type = "covariance", lag.max = 100, ylim = c(-0.0015,0.0015))
acf(sibm_ts, type = "correlation", lag.max = 100, ylim = c(-0.15,0.15))
acf(sibm_ts, type = "partial", lag.max = 100, ylim = c(-0.15,0.15))
quartz()
par(mfrow = c(3,1))
acf(libm_ts, type = "covariance", lag.max = 100, ylim = c(-0.0015,0.0015))
acf(libm_ts, type = "correlation", lag.max = 100, ylim = c(-0.15,0.15))
acf(libm_ts, type = "partial", lag.max = 100, ylim = c(-0.15,0.15))

# Test for white noise using Ljung-Box test
Box.test(sibm_ts, lag = 5, type = "Ljung-Box") 
# Notice the large Q-stat p-value = 0.64! --> Fail to reject H0 
Box.test(sibm_ts, lag = 10, type = "Ljung-Box")
# Notice the large Q-stat p-value = 0.17! --> Fail to reject H0
# Conclusion: From the ACF we can see very small(~0) serial corrleations of monthly IBM stock returns from Jan 1926 -Dec 2008. 
# The ACFs are within the 2 Std. Error limits --> Not statisticaly different from 0 at the 5% level. 


# Series II: Monthly Value-Weighted Index (S&P 500) Returns from 1926-2008
vwrtn_ts = ts(z[,3], frequency = 12, start = c(1926,1))
quartz()
plot(stl(vwrtn_ts, s.window = "periodic"))
quartz()
tsdisplay(vwrtn_ts) 

# Autocovariance, ACF, and PACF Functions
quartz()
par(mfrow = c(3,1))
acf(vwrtn_ts, type = "covariance", lag.max = 100, ylim = c(-0.0015,0.0015))
acf(vwrtn_ts, type = "correlation", lag.max = 100, ylim = c(-0.15,0.15))
acf(vwrtn_ts, type = "partial", lag.max = 100, ylim = c(-0.15,0.15))

# Test for white noise using Ljung-Box test
Box.test(vwrtn_ts, lag = 5, type = "Ljung-Box") 
# Notice the low Q-stat p-value ~0 --> Reject Ho 
Box.test(vwrtn_ts, lag = 10, type = "Ljung-Box")
# Notice the low Q-stat p-value ~0 --> Reject Ho
# Conclusion: From the ACF we can see that monthly market index returns are serially corrleated. Therefore, market indices have stronger serial dependence than individual stock returns. 

# Plots
par(mfrow = c(1,1))
plot(sibm_ts, col = "blue", xlab = "Time", ylab = "Returns", main = "IBM vs. SP500 Monthly Simple Return")
lines(vwrtn_ts, col = "red")
legend(x = "topright", legend = c("IBM", "SP500"), lty = c(1,1), col = c("blue", "red"), cex = 0.75, inset = 0.05)

