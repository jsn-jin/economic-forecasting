...
plot(caemp_ts, xlab = 'Year', ylab = "Canadian Employment", lwd = 2)
grid()
lines(ma1$fitted.values, col = "red", lwd = 2)
lines(ma2$fitted.values, col = "seagreen2", lwd = 2)
lines(ma3$fitted.values, col = "blue", lwd = 2)
legend("topright", legend = c("Data", "MA(1)","MA(2)","MA(10)"), text.col = 1:4, bty = "n")
...
# ACF has some significant spikes

# AR(p) Model Fitting (see page 157)
ar1 = ar(caemp_ts, aic = FALSE, order.max = 1) # AR(1), FALSE is needed to allow for different values of p 
ar1
ar2 = ar(caemp_ts, aic = FALSE, order.max = 2) # AR(2), Note: if no set FALSE, it will figure the best order (p) 
ar2
ar3 = ar(caemp_ts, aic = FALSE, order.max = 3) # AR(3) 
ar3
ar(caemp_ts, aic = TRUE) # order selected = 2


# You can also use ARMA(p,q) but setting q=0 this time.
ar1 = arma(caemp_ts, order = c(1,0)) # ARMA(1,0) = AR(1) = AR(1) + MA(0)
summary(ar1)
ar2 = arma(caemp_ts, order = c(2,0)) # ARMA(2,0) = AR(2) = AR(2) + MA(0)
summary(ar2)
ar3 = arma(caemp_ts, order = c(3,0)) # ARMA(3,0) = AR(3) = AR(3) + MA(0)
summary(ar3)
...
lines(ar3$fitted.values, col = "blue", lwd = 2, lty = 2)
legend("topright", legend=c("Data", "AR(1)", "AR(2)", "AR(3)"), text.col = 1:4, bty = "n")
# Model		AIC
# AR(1)		519
# AR(2)		486
# AR(3)		488
...
acf(ar2$residuals[3:136], type = "correlation", main = "Autocorrelation", lag.max = 25, ylab = "ACF")
acf(ar2$residuals[3:136], type = "partial", main = "Partial Autocorrelation", lag.max = 25,  ylab = "PACF")
...
plot(caemp_ts, xlab = 'Year', ylab = "Canadian Employment", lwd = 2)
grid()
lines(arma1$fitted.values, col = "red", lwd = 2, lty = 2)
lines(arma2$fitted.values, col = "seagreen2", lwd = 2, lty = 2)
lines(arma3$fitted.values, col = "blue", lwd = 2, lty = 2)
lines(arma4$fitted.values, col = "cyan", lwd = 2, lty = 2)
legend("topright", legend = c("Data", "ARMA(1,1)", "ARMA(1,2)", "ARMA(2,1)", "ARMA(2,2)"), text.col = 1:5, bty = "n")
...
auto.arima(caemp_ts)
# ARIMA(1,1,0)
# The 1 in the middle means you should take the first difference


# Simulate AR(p), MA(q), and ARMA(p,q) process and test the theory :)
# arima.sim() function from stats package
...
ar.sim <- arima.sim(model = list(ar = c(0.9, -0.2, -0.8, 0.5)), n = 1000) # Note: 'ar' has 4 coefficients, hence AR(4)
quartz()
par(mfrow = c(3,1))
plot(ar.sim)
acf(ar.sim)
...
# 3. Simulate an ARMA(2,2) process. According to theory,...?? 
arma.sim <- arima.sim(model = list(ar = c(0.9, -0.2), ma = c(-0.7, 0.1)), n = 400)
tsdisplay(arma.sim)
quartz()
par(mfrow = c(3,1))
plot(arma.sim)
